...
{
  int socket_source, socket_sink, cc, fsize; // for the sockets & reciver
  ...
  socket_source = socket (AF_INET, SOCK_DGRAM, 0); // open a new UDP socket (without stating the protocol and returning its ptr) using port P+1 (sender)
  ...
  socket_sink = socket (AF_INET, SOCK_DGRAM, 0); // opens a new socket using port P (receiver)
  ...
  bind(socket_sink, (struct sockaddr *)&s_in, sizeof(s_in)); // it checks if the port is available
  ...
  for(;;) // this loop will continue until we kill the process (Ctrl+C) - waiting until gets a datagram from source 
  {
    // A message reciever:
    fsize = sizeof(from);
    cc = recvfrom(socket_sink,&msg,sizeof(msg),0,(struct sockaddr *)&from,&fsize); // waiting for another incoming datagram - WAITING CUBE (using port P)
    ...
    if(rand_value > threshold)
    {
      ...
      sendto(socket_source,&msg,sizeof(msg),0,(struct sockaddr *)&dest, sizeof(dest)); // SEND IT (using port P+1)
    }
