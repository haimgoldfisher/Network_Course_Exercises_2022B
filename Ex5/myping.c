...
int main ()
{
    ...
    {
        fprintf(stderr, "failed in sendto() with error: %d", errno);
        return -1;
    }
    printf("Ping send to IP: %s successfully\n", DESTINATION_IP);
    while (1)
    {
        int dest_in_len = sizeof(dest_in);
        char buff[IP_MAXPACKET] = {0};
        if(recvfrom(sock, buff, sizeof(buff), 0, (struct sockaddr *)&dest_in, (socklen_t *)&dest_in_len) == -1)
        {
        ...
        else
        {
            ...
            double curr_micros =(t2.tv_sec - t1.tv_sec) + (t2.tv_nsec - t1.tv_nsec)/1000.0;
            double curr_milis = curr_micros / 1000.0;
            printf("Received Echo answer from IP: %s\n", DESTINATION_IP);
